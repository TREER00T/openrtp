    ...
    tagItem.data.forEach((object, index) => {
        ...
        arrayOfHtmlTags.push(itemInSection(on, isLastIndex, viewId, isDefined(security),
            tag.detailItem(route, description, viewId, getNumberForListOfItem([route, description, parameters, events]),
                arrToString([parametersAndArgsObjectValidation(parameters, viewId), eventValidationObject(events, viewId)]))));
...
function parametersAndArgsObjectValidation(data, viewId) {
    let parameters = [],
        ...
        dataArgsObject = () => args.join(' '),
        isUsedArgs = () => args.length !== 0,
    ...
    data?.forEach(item => {
       ...
       // console.log(item)
        ...
        if (isArgsUsed)
            return args.push(rowsAndColumnsForArgs(argsItem));
    ...
    if (isUsedArgs())
        arrayOfHtmlTag.push(detailsForArgs(viewId, isUsedArgs(), dataArgsObject()));
...
function eventValidationObject(data, viewId) {
    let arrayOfHtmlTag = [];
    data?.forEach(item => {
